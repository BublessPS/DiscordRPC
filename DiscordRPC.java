...
/**
 * Core library binding for the official
 * <a href="https://github.com/discordapp/discord-rpc" target="_blank">Discord
 * RPC SDK</a>. <br>
...
public interface DiscordRPC extends Library {
	...
	/**
	 * Currently unsused response, treated like NO. Used with
	 * {@link #Discord_Respond(String, int)}
	 * 
	 * @see #DISCORD_REPLY_NO
	 */
	 ...
	 * Handlers are only called when the {@link #Discord_RunCallbacks()} method is
	 * invoked! <br>
	 * <b>Before closing the application it is recommended to call
	 * {@link #Discord_Shutdown()}</b>
	 * 
	 * @param applicationId The ID for this RPC application, retrieved from the
	 *                      <a href=
	 *                      "https://discordappc.com/developers/applications/me"
	 *                      target="_blank">developer dashboard</a>
	 * @param handlers      Nullable instance of
	 *                      {@link club.minnced.discord.rpc.DiscordEventHandlers}
	 * @param autoRegister  {@code true} to automatically call
	 *                      {@link #Discord_RegisterSteamGame(String, String)} or
	 *                      {@link #Discord_Register(String, String)}
	 * @param steamId       Possible steam ID of the running game
	 */
	void Discord_Initialize(@Nonnull String applicationId, @Nullable DiscordEventHandlers handlers,
			boolean autoRegister, @Nullable String steamId);
	...
	/**
	 * Executes the registered handlers for currently queued events. <br>
	...
	/**
	 * Polls events from the RPC pipe and pushes the currently queued presence. <br>
	 * This will be performed automatically if the attached binary has an enabled IO
	 * thread (default)
	 * 
	 * <p>
	 * <b>If the IO-Thread has been enabled this will not be supported!</b>
	 */
	...
	/**
	 * Registers the given application so it can be run by the discord client.
	 * {@code discord-<appid>://}
	 * 
	 * @param applicationId The ID of the application to register
	 * @param command       The command for the application startup, or {@code null}
	 *                      to use the current executable's path
	 */
